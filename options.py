...
def get_options(option_type):
  ...
  tf.app.flags.DEFINE_integer("vis_h", 21, "input image height in pixels")
  tf.app.flags.DEFINE_integer("vis_w", 21, "input image width in pixels")
  tf.app.flags.DEFINE_string("checkpoint_dir", "/tmp/StRADRL/checkpoints", "checkpoint directory")

  # For training
  if option_type == 'training':
    tf.app.flags.DEFINE_string("training_name","mazepro_noaux_5e5_lambda05","name of next training in log")
    ...
    tf.app.flags.DEFINE_float("base_lambda", 1.0, "generalized adv. est. lamba for short-long sight")
